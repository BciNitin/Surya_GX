...
{
    public class ReprintingFactory
    {
        #region Inward Order
        ...
        public List<InventoryEntriesForReprint> FetchInventorysForReport(ReprintingSearchCriteria criteria)
        {
            List<InventoryEntriesForReprint> Inventerys = new List<InventoryEntriesForReprint>();
            using (SqlConnection cn = new SqlConnection(DBConfig.SQLConnectionString))
            {
                cn.Open();
                using (var cm = cn.CreateCommand())
                {
                    ...
                    using (SafeDataReader dr = new SafeDataReader(cm.ExecuteReader()))
                    {
                        while (dr.Read())
                        {
                            InventoryEntriesForReprint zone = GetInventery(dr);
                            Inventerys.Add(zone);
                        }
                    }
                }
        ...
        private InventoryEntriesForReprint GetInventery(SafeDataReader dr)
        {
            InventoryEntriesForReprint inventery = new InventoryEntriesForReprint();
