...
@Component({
    selector: 'app-log-forms-list',
    templateUrl: './log-forms-list.component.html',
    animations: [appModuleAnimation()],
    styleUrls: ['./log-forms-list.component.css']
})

export class LogFormsListComponent extends PagedListingComponentBase<ClientFormsDto> {
        ...
        this._clientFormsService.get(ID).subscribe((res: ClientFormsDto) => {
            if (res) {
                this.modules = {
                    ...
                    "approveDateTime": res.approveDateTime,
                    "id": ID
                }
                this._clientFormsService.update(this.modules).subscribe((res: ClientFormsDto) => {
                    if (res) {
                        abp.notify.success(this.l('This Log has been send for Approved.'));
                        abp.ui.clearBusy();
                        this._router.navigate(['./log-form-list'], { relativeTo: this._route });
